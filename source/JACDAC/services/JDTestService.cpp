...
{
    DMESG("REC N: %d F: %d->%d N: %s M: %c%c%c", *((uint32_t*)p->data), p->device_identifier, p->service_number, this->name.toCharArray(), (this->mode == ClientService) ? "C":"", (this->mode == HostService) ? "H":"", (this->mode == BroadcastService) ? "B":"");
    return DEVICE_OK;
}
...
void JDTestService::sendTestPacket(uint32_t value)
{
    if (!this->device)
        return;
    DMESG("SEND: %d FROM: %d", value, this->device->device_identifier);
